...
def chat(query: str, session_id: str, all_sessions: Dict[str, Session]) -> str:
    ...
    # ------------------------------------------------------------------
    # 3. Choose processing path based on module type
    # ------------------------------------------------------------------
    try:
        if current_module.type == ModuleType.AGENT:
            # AGENT module: go through the agent loop
            response = _handle_agent_module(session, current_module)
        else:
            # Invoke pattern-registered stages for pipeline processing
            response = _run_pipeline(session, current_module)
    except Exception as e:
...
def _handle_agent_module(session: Session, module) -> str:
    """Agent loop handling for AGENT modules.

    Calls conversation() in loop.py, which builds system_prompt from module,
    ...
    Returns:
        str: reply text generated by the LLM
    """
    from src.chat.loop import conversation

    logger.info("Agent 模块处理: module=%s", module.module_code)
    return conversation(session, module, session.cxt.llm_config)
...
def _run_pipeline(session: Session, module) -> str:
    ...
    Returns:
        str: reply text generated by NLG
    """
    cxt = session.cxt
    pattern = session.pattern
    ...
    # ------------------------------------------------------------------
    # 4. After processing, perform node transition
    # ------------------------------------------------------------------
...
def _handle_node_transition(cxt, module) -> None:
    ...
    # ROUTE type: dispatch to the menu node's jump_module when declared,
    # otherwise reset to root (router stays at root between turns)
    if module.type == ModuleType.ROUTE:
        cur_node = (
            cxt.node_map.get(cxt.current_node_code)
            if cxt.current_node_code
            else None
        )
        jump_module_code = (
            getattr(cur_node, "jump_module", None) if cur_node is not None else None
        )

        if jump_module_code and jump_module_code in cxt.module_map:
            logger.info(
                "ROUTE 分发: 模块 %s → 模块 %s（下一轮从子模块首节点开始）",
                module.module_code,
                jump_module_code,
            )
            cxt.current_module_code = jump_module_code
